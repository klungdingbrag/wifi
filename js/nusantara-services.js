...
const state={installed:true,version:'1.5.2',requests:0,successes:0,failures:0,lastRequestAt:0,lastSuccessAt:0,lastFailureAt:0,lastError:''};
...
async function execute(moduleName,operation,requestFn,options={}){if(typeof requestFn!=='function')throw new Error('Service operation tidak tersedia.');state.requests++;state.lastRequestAt=now();const started=state.lastRequestAt;try{const data=normalize(await requestFn());state.successes++;state.lastSuccessAt=now();state.lastError='';const payload={success:true,module:moduleName,operation,data,requestedAt:started,completedAt:state.lastSuccessAt,cached:!!options.cached};return moduleName==='absensi'&&data&&typeof data==='object'&&data.success===true?data:payload}catch(error){const err=toError(error,'Request service gagal.');state.failures++;state.lastFailureAt=now();state.lastError=err.message;err.service=moduleName;err.operation=operation;throw err}}
