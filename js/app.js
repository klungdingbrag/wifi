...
async function apiGet(action, params = {}) {
  ...
  let data;
  try { data = JSON.parse(text); } catch (e) { throw new Error('Respons backend bukan JSON. Periksa deployment Web App Apps Script.'); }
...
const PAGE_META = { dashboard:['OVERVIEW','Dashboard'], tagihan:['BILLING','Tagihan'], pelanggan:['CUSTOMERS','Pelanggan'], pembayaran:['PAYMENTS','Pembayaran'], histori:['AUDIT TRAIL','Histori'] };

document.addEventListener('DOMContentLoaded',()=>{ try { bindEvents(); setDefaultPeriods(); loadInitialData(); } catch(e){ console.error(e); showFatalError('Frontend gagal diinisialisasi: '+errorMessage(e)); }});
...
function setDefaultPeriods(){ const p=periodNow(); $('billPeriod').value=p; $('generatePeriod').value=p; }
...
function loadInitialData(){
  ...
  apiGet('getInitialData').then(data=>{
    ...
    console.error(err); setConnection('bad'); $('loadingScreen').classList.add('hidden'); $('appShell').classList.remove('hidden');
...
function showPage(page){ if(!PAGE_META[page])return; APP.currentPage=page; document.querySelectorAll('.page').forEach(p=>p.classList.remove('active')); const el=$('page-'+page); if(el)el.classList.add('active'); document.querySelectorAll('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.page===page)); $('pageEyebrow').textContent=PAGE_META[page][0]; $('pageTitle').textContent=PAGE_META[page][1]; $('sidebar').classList.remove('open'); window.scrollTo({top:0,behavior:'smooth'}); }
...
function isBillOperational(bill){ const c=findCustomer(bill['ID Pelanggan']); return !!c && String(c['Status'])==='Aktif'; }

function renderDashboard(){
  const active=APP.pelanggan.filter(x=>String(x['Status'])==='Aktif');
  const activeIds=activeCustomerMap();
  ...
  $('dashboardOutstanding').innerHTML=unpaid.length?unpaid.slice(0,6).map(b=>`<div class="outstanding-item"><div><div class="item-name">${esc(b['Nama'])}</div><div class="item-meta">${esc(b['Periode'])} · jatuh tempo ${dateShort(b['Jatuh Tempo'])}</div></div><div class="item-money">${money(b['Nominal'])}<small>Belum bayar</small></div></div>`).join(''):`<div class="empty-state"><div class="empty-icon">✓</div><strong>Semua tagihan aman</strong><span>Tidak ada tagihan aktif yang perlu ditindaklanjuti.</span></div>`;
}

function renderCustomers(){
  const q=String($('customerSearch').value||'').toLowerCase().trim(), status=$('customerStatus').value;
  const list=APP.pelanggan.filter(c=>{const hay=[c['ID Pelanggan'],c['Nama Pelanggan'],c['No WhatsApp'],c['Paket Speed'],c['Alamat'],c['Catatan']].join(' ').toLowerCase();return(!q||hay.includes(q))&&(!status||c['Status']===status);});
  $('customerEmpty').classList.toggle('hidden',list.length!==0);
  $('customerGrid').innerHTML=list.map(c=>{const active=c['Status']==='Aktif';return `<article class="customer-card"><div class="customer-top"><div class="customer-avatar">${esc(initialsOf(c['Nama Pelanggan']))}</div><span class="badge ${active?'badge-active':'badge-inactive'}">${active?'Aktif':'Nonaktif'}</span></div><div class="customer-name">${esc(c['Nama Pelanggan'])}</div><div class="customer-id">${esc(c['ID Pelanggan'])}</div><div class="customer-info"><div class="info-row"><span>Paket</span><span>${esc(c['Paket Speed']||'-')}</span></div><div class="info-row"><span>Tarif</span><span>${money(c['Tarif Bulanan'])}</span></div><div class="info-row"><span>WhatsApp</span><span>${esc(c['No WhatsApp']||'-')}</span></div><div class="info-row"><span>Alamat</span><span>${esc(shortText(c['Alamat'],28)||'-')}</span></div></div><div class="customer-actions"><button class="btn btn-soft btn-mini" onclick="openCustomerDetail(${Number(c._rowIndex)})">Detail</button><button class="btn btn-success btn-mini" onclick="sendCustomerWA(${Number(c._rowIndex)})">WhatsApp</button><button class="btn btn-soft btn-mini" onclick="openEditCustomer(${Number(c._rowIndex)})">Edit</button></div></article>`;}).join('');
}

function openCustomerModal(rowIndex=null){
  $('customerForm').reset();$('customerRowIndex').value='';$('customerModalTitle').textContent='Pelanggan Baru';$('customerSaveBtn').textContent='Simpan pelanggan';$('deleteCustomerBtn').classList.add('hidden');$('fStatus').value='Aktif';$('fTanggal').value=todayInput();
  if(rowIndex){const c=findCustomerByRow(rowIndex);if(!c)return;$('customerRowIndex').value=c._rowIndex;$('customerModalTitle').textContent='Edit Pelanggan';$('customerSaveBtn').textContent='Simpan perubahan';$('deleteCustomerBtn').classList.remove('hidden');$('fNama').value=c['Nama Pelanggan']||'';$('fWhatsApp').value=c['No WhatsApp']||'';$('fPaket').value=c['Paket Speed']||'';$('fTarif').value=num(c['Tarif Bulanan']);$('fTanggal').value=dateInput(c['Tanggal Pasang']);$('fStatus').value=c['Status']||'Aktif';$('fAlamat').value=c['Alamat']||'';$('fCatatan').value=c['Catatan']||'';}
  $('customerModal').classList.remove('hidden');
}
function openEditCustomer(rowIndex){openCustomerModal(rowIndex);}
function saveCustomer(e){
  e.preventDefault();const data={rowIndex:Number($('customerRowIndex').value)||null,nama:$('fNama').value.trim(),noWhatsApp:$('fWhatsApp').value.trim(),paketSpeed:$('fPaket').value.trim(),tarifBulanan:Number($('fTarif').value||0),tanggalPasang:$('fTanggal').value,status:$('fStatus').value,alamat:$('fAlamat').value.trim(),catatan:$('fCatatan').value.trim()};
  if(!data.nama)return toast('Nama pelanggan wajib diisi.',true);if(!data.noWhatsApp)return toast('Nomor WhatsApp wajib diisi.',true);if(data.tarifBulanan<0)return toast('Tarif bulanan tidak valid.',true);
  const btn=$('customerSaveBtn');btn.disabled=true;apiPost(data.rowIndex?'updateCustomer':'addCustomer',data).then(result=>{btn.disabled=false;closeModal('customerModal');toast(result?.message||'Data pelanggan tersimpan.');loadInitialData();}).catch(err=>{btn.disabled=false;toast(errorMessage(err),true);});
}
function deleteCurrentCustomer(){
  const rowIndex=Number($('customerRowIndex').value);if(!rowIndex)return;const c=findCustomerByRow(rowIndex);if(!c)return toast('Data pelanggan tidak ditemukan.',true);const name=c['Nama Pelanggan']||'pelanggan ini';if(!confirm('Hapus '+name+'?\n\nData pelanggan akan dihapus dari daftar pelanggan.\nHistori tagihan dan pembayaran TIDAK akan dihapus.'))return;const btn=$('deleteCustomerBtn');btn.disabled=true;apiPost('deleteCustomer',{rowIndex}).then(result=>{btn.disabled=false;closeModal('customerModal');toast(result?.message||'Pelanggan berhasil dihapus.');loadInitialData();}).catch(err=>{btn.disabled=false;toast(errorMessage(err),true);});
}
...
function openCustomerDetail(rowIndex){
  const c=findCustomerByRow(rowIndex);if(!c)return;$('detailName').textContent=c['Nama Pelanggan']||'Detail Pelanggan';
...
function renderBills(){
  const q=String($('billSearch').value||'').toLowerCase().trim(),status=$('billStatus').value,period=$('billPeriod').value,activeIds=getActiveCustomerIds();
  const list=APP.tagihan.filter(b=>{const hay=[b['ID Tagihan'],b['ID Pelanggan'],b['Nama'],b['Periode']].join(' ').toLowerCase();return activeIds.has(String(b['ID Pelanggan']))&&String(b['Periode'])===String(period)&&(!q||hay.includes(q))&&(!status||b['Status']===status);});
...
function openPaymentModal(rowIndex){const b=findBillByRow(rowIndex);if(!b)return;if(String(b['Status'])==='Lunas')return toast('Tagihan ini sudah lunas.',true);if(!isBillOperational(b))return toast('Pelanggan ini sudah tidak aktif.',true);$('paymentRowIndex').value=b._rowIndex;$('paymentMethod').value='Tunai';$('paymentNote').value='';$('paymentSummary').innerHTML=`<strong>${esc(b['Nama'])}</strong><span>${esc(b['ID Tagihan'])} · ${esc(b['Periode'])} · ${money(b['Nominal'])}</span>`;$('paymentModal').classList.remove('hidden');}
function savePayment(e){e.preventDefault();const row=Number($('paymentRowIndex').value),method=$('paymentMethod').value,note=$('paymentNote').value.trim();const btn=$('paymentSaveBtn');btn.disabled=true;apiPost('payBill',{rowIndex:row,method,note}).then(result=>{btn.disabled=false;closeModal('paymentModal');toast(result?.message||'Pembayaran tersimpan.');loadInitialData();}).catch(err=>{btn.disabled=false;toast(errorMessage(err),true);});}
function openGenerateModal(){$('generatePeriod').value=$('billPeriod').value||periodNow();$('generateModal').classList.remove('hidden');}
function generateBills(){const period=$('generatePeriod').value;if(!period)return toast('Pilih periode terlebih dahulu.',true);const btn=$('confirmGenerate');btn.disabled=true;apiPost('generateMonthlyBills',{requestedPeriod:period}).then(result=>{btn.disabled=false;closeModal('generateModal');$('billPeriod').value=period;toast(result?.message||'Tagihan berhasil dibuat.');loadInitialData();}).catch(err=>{btn.disabled=false;toast(errorMessage(err),true);});}
...
function buildBillMessage(bill,customer){const biz=APP.pengaturan.NAMA_BISNIS||'Nusantara WiFi';const bank=APP.pengaturan.BANK||'';const account=APP.pengaturan.NO_REKENING||'';const ewallet=APP.pengaturan.E_WALLET||'';let paymentInfo='';if(bank&&account)paymentInfo+=`\nTransfer ${bank}: ${account}`;if(ewallet)paymentInfo+=`\nE-Wallet: ${ewallet}`;return `Halo ${customer?.['Nama Pelanggan']||bill['Nama']},\n\nSalam dari ${biz}.\n\nInformasi tagihan WiFi:\n\nID Tagihan: ${bill['ID Tagihan']}\nPeriode: ${bill['Periode']}\nPaket: ${customer?.['Paket Speed']||'-'}\nNominal: ${money(bill['Nominal'])}\nJatuh tempo: ${dateShort(bill['Jatuh Tempo'])}\nStatus: ${bill['Status']}${paymentInfo}\n\nMohon melakukan pembayaran sebelum jatuh tempo.\n\nTerima kasih.`;}
function sendCustomerWA(rowIndex){const c=findCustomerByRow(rowIndex);if(!c)return;const phone=normalizeWA(c['No WhatsApp']);if(!phone)return toast('Nomor WhatsApp pelanggan belum tersedia.',true);const period=$('billPeriod').value||periodNow();let bill=APP.tagihan.find(b=>b['ID Pelanggan']===c['ID Pelanggan']&&String(b['Periode'])===String(period));if(!bill){bill=APP.tagihan.filter(b=>b['ID Pelanggan']===c['ID Pelanggan']).sort((a,b)=>String(b['Periode']).localeCompare(String(a['Periode'])))[0];}if(!bill)return sendWA(phone,`Halo ${c['Nama Pelanggan']},\n\nSalam dari ${APP.pengaturan.NAMA_BISNIS||'Nusantara WiFi'}.\n\nTagihan Anda belum tersedia di sistem.\n\nTerima kasih.`);sendWA(phone,buildBillMessage(bill,c));}
function sendBillWA(rowIndex){const b=findBillByRow(rowIndex);if(!b)return;const c=findCustomer(b['ID Pelanggan']);if(!c)return toast('Data pelanggan untuk tagihan ini tidak ditemukan.',true);const phone=normalizeWA(c['No WhatsApp']);if(!phone)return toast('Nomor WhatsApp pelanggan belum tersedia.',true);sendWA(phone,buildBillMessage(b,c));}
...
function renderPayments(){const list=APP.pembayaran.slice().sort((a,b)=>String(b['Tanggal Bayar']).localeCompare(String(a['Tanggal Bayar'])));$('paymentEmpty').classList.toggle('hidden',list.length!==0);$('paymentTableBody').innerHTML=list.map(p=>`<tr><td><span class="primary-text">${esc(p['ID Pembayaran'])}</span></td><td><div class="primary-text">${esc(p['Nama Pelanggan'])}</div><div class="muted-text">${esc(p['ID Pelanggan'])}</div></td><td>${esc(p['Periode'])}</td><td class="money">${money(p['Nominal'])}</td><td>${dateShort(p['Tanggal Bayar'])}</td><td>${esc(p['Metode'])}</td></tr>`).join('');}
